...
const char outputname[] = "Q95headerout_fumi.bin";
FILE *outfp;
...
int main(void) {
    ...
    for (int i = 0; i < 8 * 8; i++) {
        ...
        // }
        quantLuminance[i] =
            clamp((float)((100 - QUALITY) / 50.0 * DefaultQuantLuminance[i]) + 0.5, 1, 255);
        quantChrominance[i] =
            clamp((float)((100 - QUALITY) / 50.0 * DefaultQuantChrominance[i]) + 0.5, 1, 255);
